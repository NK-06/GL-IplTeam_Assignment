...
//		        out.print(st.getTeamId());
//		        out.print(st.getOwnScore());
		        ...
		        if(st.getTeamId().equalsIgnoreCase(st.getOppId()))    // st = object of Summary Table
		        {
		     	   ...
		     	   request.getRequestDispatcher("IplUpdateTable.jsp").include(request, response);
		             	...
		             	List<PointTable> pt1 = Dao.table();                    //pt = list of Point Table type having complete Point table
		        	  
		             	checkExist(st.getTeamId(), pt1);
		             	checkExist(st.getOppId(), pt1);
